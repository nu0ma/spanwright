...
func TestValidateTableName(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Valid table name with select in it",
			tableName: "users_select",
			wantErr:   false,
		},
		{
			name:      "Valid table name with drop in it",
			tableName: "users_drop",
			wantErr:   false,
		},
		{
			name:      "Valid table name with delete in it",
			tableName: "users_delete",
			wantErr:   false,
		},
		{
			name:      "Valid table name with update in it",
			tableName: "users_update",
			wantErr:   false,
		},
		{
			name:      "Valid table name with insert in it",
			tableName: "users_insert",
			wantErr:   false,
		},
		{
			name:      "Valid table name with union in it",
			tableName: "users_union",
			wantErr:   false,
		},
		{
			name:      "Valid table name with exec in it",
			tableName: "users_exec",
			wantErr:   false,
		},
		{
			name:      "Valid table name with script in it",
			tableName: "users_script",
			wantErr:   false,
		},
		{
			name:      "SQL injection attempt - SELECT with space",
			tableName: "users select * from",
			wantErr:   true,
		},
		{
			name:      "SQL injection attempt - semicolon SELECT",
			tableName: "users;select",
			wantErr:   true,
		},
		{
			name:      "SQL injection attempt - DROP with space",
			tableName: "users drop table",
			wantErr:   true,
		},
		{
			name:      "XSS attempt - script tag",
			tableName: "users<script>alert",
			wantErr:   true,
		},
		{
			name:      "XSS attempt - javascript protocol",
			tableName: "users_javascript:alert",
			wantErr:   true,
		},
	}
