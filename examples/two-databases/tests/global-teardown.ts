...
async function globalTeardown() {
  ...
  try {
    ...
    if (fs.existsSync(testResultsDir)) {
      ...
      for (const configFile of configFiles) {
        ...
        try {
          ...
          console.log(
            `🔧 Cleaned up worker ${workerConfig.workerId} (databases: ${workerConfig.primaryDbId}, ${workerConfig.secondaryDbId})`
          );
    ...
    if (stopEmulator) {
      ...
      console.log(
        '🔄 Leaving Spanner emulator running (set STOP_EMULATOR_AFTER_TESTS=true to stop)'
      );
    }
