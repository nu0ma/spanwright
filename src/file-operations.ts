...
export function setupSchemaDirectories(projectPath: string, config: any): void {
  ...
  // Create primary schema directory only if it doesn't exist
  // If path is absolute (existing schema), don't create anything
  // If path is relative (new project), create empty directory
  const primarySchemaPath = path.join(projectPath, config.primarySchemaPath);
  if (!path.isAbsolute(config.primarySchemaPath)) {
  ...
  if (config.count === '2') {
    ...
    if (!path.isAbsolute(config.secondarySchemaPath)) {
