...
describe('Configuration Module', () => {
  ...
  describe('getConfiguration', () => {
    it('should call getNonInteractiveConfiguration when isNonInteractive is true', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith('🤖 Non-interactive mode: Creating project with 1 database(s)')
    })
  ...
  describe('getNonInteractiveConfiguration', () => {
    it('should use environment variables when provided', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith('🤖 Non-interactive mode: Creating project with 2 database(s)')
      expect(mockConsoleLog).toHaveBeenCalledWith('   Primary DB: custom-primary (/custom/primary/schema)')
      expect(mockConsoleLog).toHaveBeenCalledWith('   Secondary DB: custom-secondary (/custom/secondary/schema)')
    })
    ...
    it('should use default values when environment variables are not set', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(`🤖 Non-interactive mode: Creating project with ${DEFAULTS.DB_COUNT} database(s)`)
      expect(mockConsoleLog).toHaveBeenCalledWith(`   Primary DB: ${DEFAULTS.PRIMARY_DB_NAME} (${DEFAULTS.PRIMARY_SCHEMA_PATH})`)
    })
